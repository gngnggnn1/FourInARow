...
public class FourInARowController implements Initializable {
     ...
     */
    private void displayBoard() throws FourInARowException {
    	...
    	for(int i = 0; i < Game.MAXIMUM_COLUMNS; i++) {
    		...
    		for(int j = 0; j < Game.MAXIMUM_ROWS; j++) {
    			Node node = getNodeByRowColumnIndex (j + 1, i);
    			
    			if(game.getCellStatus(i, j) == CellStatus.PLAYER_ONE) {
    				if(node instanceof Pane) {
    		            node.setStyle("-fx-background-color: " + "#00f");
    		        }
    		        else {
    		            messageArea.setText("Describe some error");
    		        }
    			}
    			if(game.getCellStatus(i, j) == CellStatus.PLAYER_TWO) {
    				if(node instanceof Pane) {
    		            node.setStyle("-fx-background-color: " + "#f00");
    		        }
    		        else {
    		            messageArea.setText("Describe some error");
    		        }
     ...
     */
    @FXML
    private void handleButtonAction(ActionEvent event) throws FourInARowException {//上边的加号
        ...
        if(game.takeTurn(selectedColumn)) messageArea.setText("Succeed.");
        else messageArea.setText("Warning !!!!! No available positions in this column.");
        displayBoard();
        if(game.hasWon()) {
        	messageArea.setText("Nice! Player " + game.getLastActivePlayer() + " Wins!!!!");
        	game.resetGame();
        }
    }
     ...
     */
    @FXML
    private void handleStartTwoPlayerGameAction(ActionEvent event) throws FourInARowException { 
     ...
     */
    @FXML
    private void handleUndoAction(ActionEvent event) throws FourInARowException {
    	if(game.undo()) messageArea.setText("Undo Succeed");
        // TODO - use the Game class to undo the most recent move. Then, redisplay the board.
        else messageArea.setText("Can't undo, please take turn first");
    	displayBoard();
    }
     ...
     */
    @FXML
    private void handleRedoAction(ActionEvent event) throws FourInARowException {
    	if(game.redo()) messageArea.setText("Redo Succeed");
        else messageArea.setText("Can't redo, please undo first");
    	displayBoard();
    }
